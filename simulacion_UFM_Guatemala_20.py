...
class Virus():
    def __init__(self, params):
        ...
        self.axes.set_title('COVID-19 UFM Guatemala 20 Infectados Iniciales', loc = 'center', y = 1, pad = 14)
        self.day_text = self.axes.annotate("Día 0", xy = [np.pi / 2, 1], ha = "center", va = "bottom")
        self.infected_text = self.axes.annotate("Infectados: 0", xy = [3 * np.pi / 2, 1], ha = "center", va = "top", color = RED)
        self.deaths_text = self.axes.annotate("\nMuertes: 0", xy = [3 * np.pi / 2, 1], ha = "center", va = "top", color = BLACK)
    ...
    def initial_population(self):
        population = 4000
        self.num_currently_infected = 20
        self.total_num_infected = 20
        ...
        for i in range(20):
            ...
            self.mild[self.mild_fast]["thetas"].append(self.thetas[i])
            self.mild[self.mild_fast]["rs"].append(self.rs[i])


    def spread_virus(self, i):
        self.exposed_before = self.exposed_after
        if self.day % self.serial_interval == 0 and self.exposed_before < 4000:
            self.num_new_infected = round(self.r0 * self.total_num_infected)
            self.exposed_after += round(self.num_new_infected * 1.1)
            if self.exposed_after > 4000:
                self.num_new_infected = round((4000 - self.exposed_before) * 0.9)
                self.exposed_after = 4000
    ...
    def update_text(self):
        self.day_text.set_text("Día {}".format(self.day))
        self.infected_text.set_text("Infectados: {}".format(self.num_currently_infected))
        self.deaths_text.set_text("\nMuertes: {}".format(self.num_deaths))
        self.recovered_text.set_text("\n\nRecuperados: {}".format(self.num_recovered))
